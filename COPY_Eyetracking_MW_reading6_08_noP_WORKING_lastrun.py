...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\BAR Lab\\Desktop\\Leilani MW Eyetracking\\COPY_Eyetracking_MW_reading6_08_noP_WORKING_lastrun.py',
...
# --- Initialize components for Routine "Calibration_reminder" ---
cal_reminder = visual.TextStim(win=win, name='cal_reminder',
    text='Please get ready for an eyetracker calibration round.',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
# --- Prepare to start Routine "Calibration_reminder" ---
continueRoutine = True
routineForceEnded = False
# update component parameters for each repeat
# keep track of which components have finished
Calibration_reminderComponents = [cal_reminder]
for thisComponent in Calibration_reminderComponents:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
# --- Run Routine "Calibration_reminder" ---
while continueRoutine and routineTimer.getTime() < 2.0:
    ...
    # *cal_reminder* updates
    if cal_reminder.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        cal_reminder.frameNStart = frameN  # exact frame index
        cal_reminder.tStart = t  # local t and not account for scr refresh
        cal_reminder.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(cal_reminder, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'cal_reminder.started')
        cal_reminder.setAutoDraw(True)
    if cal_reminder.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > cal_reminder.tStartRefresh + 2.0-frameTolerance:
            # keep track of stop time/frame for later
            cal_reminder.tStop = t  # not accounting for scr refresh
            cal_reminder.frameNStop = frameN  # exact frame index
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'cal_reminder.stopped')
            cal_reminder.setAutoDraw(False)
...
# --- Ending Routine "Calibration_reminder" ---
for thisComponent in Calibration_reminderComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
